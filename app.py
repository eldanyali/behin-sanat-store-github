...
app = Flask(__name__)
app.secret_key = os.environ.get('SECRET_KEY', 'development-key-change-before-deployment')
            ...
            if result:
                ...
                if password == oldPassword:
                    try:
                        ...
                        conn.rollback()
                        msg = "تغییر رمز عبور انجام نشد."
                else:
                    msg = "رمز عبور فعلی نادرست است."
            else:
                msg = "کاربر پیدا نشد."
        ...
        if result:
            ...
            cur.execute("""
                SELECT products.productId, products.name, products.price,
                       products.image, COUNT(kart.productId) AS quantity
                FROM products
                JOIN kart ON products.productId = kart.productId
                WHERE kart.userId = ?
                GROUP BY products.productId, products.name, products.price, products.image
            """, (userId,))
            products = cur.fetchall()
        else:
            products = []
...
def saveorder():
    ...
    # Get current Iran timestamp
    iran_timezone = pytz.timezone('Asia/Tehran')
    order_date = datetime.now(iran_timezone).strftime('%Y-%m-%d %H:%M:%S')
    with sqlite3.connect('database.db') as conn:
        cur = conn.cursor()
